...
2. Determines the branch based on the chosen strategy. `<base>` below is the name of the source repo's current `HEAD`: its checked-out branch, `HEAD` itself when it is detached or not on a local branch, or `main` when it has no commit yet:
   - **New branch:** checks for a local branch `refs/heads/<name>` first, then a remote-tracking branch `refs/remotes/origin/<name>`, then creates off `refs/remotes/origin/<base>`, or `refs/heads/<base>` when that ref does not exist. A branch it creates off the base tracks nothing (`--no-track`), whatever `branch.autoSetupMerge` says, so its first Push from `G` asks before `push -u origin` publishes it under its own name; tracking keys that a deleted branch of that name left in the repository's config (`branch.<name>.remote` and `.merge`) are removed first, and when such keys are set in a config outside the repository's own (the global config or an included file) the add stops and names the key. A tag or other ref of the same name never counts and is never checked out
