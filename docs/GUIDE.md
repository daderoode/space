...
`Enter` on the picker fetches each selected repo from `origin` and fast-forwards its local branches that are strictly behind `origin/<branch>`, or, for a branch that tracks its namesake on another remote, strictly behind that remote's `<branch>` as of that remote's last fetch (the sync fetches origin only); branches that are ahead, diverged or checked out are left as they are, and so is a branch that tracks anything else on a remote other than origin. The report has one row per repo and is titled `Sync report · N of M` while it runs, then `Sync report · N ok` or `Sync report · N ok, M failed`. It waits for `Enter`, which works once every repo is done. A failed fetch does not stop the create: `Enter` still continues, and the branch choices come from local refs, which may be behind origin. `Esc` returns to Pick Repos with the selection and query intact; a sync that is still running stops before its next git call.
