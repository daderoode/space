...
2. Determines the branch based on the chosen strategy. `<base>` below is the name of the source repo's current `HEAD`: its checked-out branch, `HEAD` itself when it is detached or not on a local branch, or `main` when it has no commit yet:
   - **New branch:** checks for a local branch `refs/heads/<name>` first, then a remote-tracking branch `refs/remotes/origin/<name>`, then creates off `refs/remotes/origin/<base>`, or `refs/heads/<base>` when that ref does not exist. A branch it creates off the base tracks nothing (`--no-track`), whatever `branch.autoSetupMerge` says, so its first Push from `G` asks before `push -u origin` publishes it under its own name; tracking keys that a deleted branch of that name left in the repository's config (`branch.<name>.remote` and `.merge`) are removed first, and when such keys are set in a config outside the repository's own (the system or global config, an included file, or a `config.worktree`) the add stops and names the key; a repository the app's git library cannot open (one with a reftable ref store, say) keeps them, as git would. A tag or other ref of the same name never counts and is never checked out
...
5. **Unreadable**: its `.git` file cannot be read or does not name a gitdir; or, read as git does not read it, it names something: its path trimmed or its first line alone names anything that is there, or a blank-separated word of that line is a worktree path of a repository that is there, which a second line, a note beside the path, or blanks around it produce; or it names, by an absolute path, something that does not exist and is not in the form git writes an absolute gitdir in (ending in `worktrees/<id>`, with no `..` and no line break), so where its source repo would be cannot be told; or it names a worktree of a directory that is not a git repository; or it names a missing git directory that leaves another repository's `worktrees` directory into a name that is not there, or through the admin directory of one of its worktrees into anything but the `modules` directory where git keeps that worktree's submodules (a note or an old path glued on after a live admin path reads like that); or it names a missing worktree path with a worktree path of a repository that is there joined on after it with no blank (an old path with the new one pasted on the end, or over its last part), which is found by reading the path again from each `/` after the missing worktree path's `worktrees`, up to 256 readings, past which the directory is kept unread; a reading that cannot be checked for a repository keeps the directory too. Not every paste is recognised, and one that is not goes as an orphan: a live path pasted on after text that is not a worktree path (such as `/old-place#now` followed by it), over the old path's `worktrees/<id>` (which leaves no `worktrees` to mark where the old path ends), or after a worktree path whose repository directory is still there but holds no repository; a dead path pasted on after a live repository's own git directory or working directory rather than after a worktree's admin path, or after the `modules` directory inside that admin path. A relative `gitdir:` that leads nowhere is kept by its own rule, described below the list
...
| `G` | Git operations for selected repo (fetch, pull, push, commit, log, rebase). Pull fetches origin and merges `origin/<name>`, or, for a branch that tracks its namesake on another remote (one made from `upstream/<name>`), fetches that remote and merges `upstream/<name>`; a branch that tracks anything else on a remote other than origin (a branch of another name, two branches, a remote the repo does not have), or whose tracking cannot be read (its remote, or on a remote other than origin its merge), is refused with a sentence saying so, and nothing runs. Push runs `git push` at once for a branch whose push destination is origin, except one that tracks a branch of another name there (below); a branch that pushes elsewhere (one made from `upstream/<name>` tracks `upstream`, and a bare push goes there) asks `Branch <name> tracks upstream/<name>. Push to upstream?` first, as does a branch whose push destination cannot be read (`where a push goes could not be read. Push anyway?`) or is the repository itself (`branch.<name>.remote` is `.`); a branch with no upstream asks before `push -u origin`, and so does a branch that tracks one branch of another name on origin and pushes there (`Branch <name> tracks origin/main. Push to origin/<name> and track it instead?`; a new-branch space made by an earlier version tracks its base, and a bare push of it is refused by git, or under `push.default=upstream` goes into origin's `main`), unless `push.default` is `current` or `remote.origin.push` is set, where the bare push already goes where they say; that push is `git -c push.default=current push -u origin refs/heads/<name>`, so it lands on origin's `<name>`, or where `remote.origin.push` maps it, whatever `push.default` says, and the branch then tracks what it landed on; all default to No |
| `PgUp` / `PgDn` | Page up / down |
...
`Enter` on the picker fetches each selected repo from `origin` and fast-forwards its local branches that are strictly behind `origin/<branch>`, or, for a branch that tracks its namesake on another remote, strictly behind that remote's `<branch>` as of that remote's last fetch (the sync fetches origin only); branches that are ahead, diverged or checked out are left as they are, and so is a branch that tracks anything else on a remote other than origin, or whose tracking cannot be read (its remote, or on a remote other than origin its merge). A branch it fast-forwards keeps what it tracks, including nothing (`git branch -f --no-track`). The report has one row per repo and is titled `Sync report · N of M` while it runs, then `Sync report · N ok` or `Sync report · N ok, M failed`. It waits for `Enter`, which works once every repo is done. A failed fetch does not stop the create: `Enter` still continues, and the branch choices come from local refs, which may be behind origin. `Esc` returns to Pick Repos with the selection and query intact; a sync that is still running stops before its next git call.
...
| Strategy | Behaviour |
|----------|-----------|
| `"new"` | Creates a new branch. Name defaults to `name` param, or set `branch` explicitly. Checks local branches first, then remote tracking, then creates off `origin/<base>`, or `<base>` without that ref, tracking nothing (see [Creating a Workspace](#creating-a-workspace)) |
