...
mod tests {
    ...
    /// The behind-only arm has no MERGE_HEAD check of its own: git's
    /// `merge --ff-only` refuses while unmerged entries exist, and this pins
    /// that it leaves the user's merge alone (probed on git 2.50.1, exit 128).
    #[test]
