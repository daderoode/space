...
**Space name**:
The name a space is created under; it becomes the directory under `workspaces.dir` and the default branch name. One plain path component: not empty, no `/` or `\`, no leading `-` or `.`, no surrounding whitespace, no control or invisible formatting characters. Interior spaces, dots and non-ASCII are fine, except the invisible joiners and marks some scripts use (a zero-width joiner in an emoji sequence, the zero-width non-joiner in Persian), which are refused because they make two names look alike. A name that breaks the rule is refused with the clause it broke, never rewritten. Looking up an existing space by name is looser (only the empty name, `.`, `..`, separators and control or formatting characters are refused), so a hand-made `-scratch` space stays viewable and removable.
...
**Unattended run**:
A git subprocess the app starts with nobody attending it: its own session with no controlling terminal, stdin closed, and a wall-clock limit after which its process group is stopped. With no terminal and no askpass helper configured, a prompt for a credential, passphrase or host key is refused rather than waited on; where the user has an askpass helper, git and ssh call that instead and the run waits on its dialog until the limit. Either way the limit is what guarantees the end. The sync runs this way in the TUI; the fetch before a worktree is created runs this way in the TUI and over MCP. The limit bounds the wait; it says nothing about whether the app stays responsive during it. In the TUI both call sites answer that the same way: each runs on a worker, so the screen keeps painting and Esc leaves at once. Over MCP there is no screen and no Esc, and the creation runs on the calling thread, so the limit is the only thing that ends it. What Esc leaves behind is where they differ. A cancelled sync leaves no half-built space behind, though the branches it had already fast-forwarded stay forwarded; a cancelled creation keeps the worktrees it had already made, and says how many in the status message.
